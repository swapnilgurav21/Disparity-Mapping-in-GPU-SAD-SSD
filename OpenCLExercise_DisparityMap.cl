...
__kernel void disparityMapping1(__read_only image2d_t d_inputL, __read_only image2d_t d_inputR,
	__global float* d_output, 
	int window_size1, int disparityMax1)
{
	...
	int windowSize = window_size1;
	int disparityMax = disparityMax1;
...
__kernel void disparityMapping2(__read_only image2d_t d_inputL, __read_only image2d_t d_inputR, 
	__global float* d_output,
	int window_size2, int disparityMax2)
{
	int i = get_global_id(0);
	int j = get_global_id(1);
	...
	int windowSize = window_size2;
	int disparityMax = disparityMax2;
	int windowRange = windowSize / 2;
	float SSD = 0.0;

...
void disparityMappingSSD(const std::vector<float>& img1, const std::vector<float>& img2, std::vector<float>& disparity, size_t width, size_t height, size_t countX, size_t countY)
{
	int windowSize = 10;
	...
	for (x = 0; x <(int) countX; x++)
	{
		for (y = 0; y <(int) countY; y++)
		{
			...
			for (disp = 0; disp <= disparityRange; disp++)
			{
				...
				{
					SSD_min = SSD;
					disparityFinal = (float)disp;
					disparityFinal = (float)(disparityFinal) / disparityRange;
				}
			}
			disparity[countX * y + x] = disparityFinal;
		}
...
int main(int argc, char** argv) {
	...
	cl::Program program = OpenCL::loadProgramSource(context, "E:/University of Stuttgart/4th Semester/Opencl-ex1/DisparityEstimation/DisparityMapEstimation.cl");
	...
	Core::readImagePGM("E:/University of Stuttgart/4th Semester/Opencl-ex1/DisparityEstimation/InImages/scene_L.pgm", inputDataL, inputWidthL, inputHeightL);
	Core::readImagePGM("E:/University of Stuttgart/4th Semester/Opencl-ex1/DisparityEstimation/InImages/scene_R.pgm", inputDataR, inputWidthR, inputHeightR);
	...
	disparityMappingSSD(h_inputL,h_inputR,h_outputCpu,inputWidthL,inputHeightL,countX, countY);
	Core::TimeSpan endTime = Core::getCurrentTime();
	Core::TimeSpan cpuTime = endTime - startTime;
	Core::writeImagePGM("E:/University of Stuttgart/4th Semester/Opencl-ex1/DisparityEstimation/OutImagesDisparity_map_cpu_scene_result.pgm", h_outputCpu, countX, countY);
